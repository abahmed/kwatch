...
type ContainerReasonsRule struct{}

func (rule ContainerReasonsRule) Detect(ctx *Context) Decision {
...
func reasonListStatus(ctx *Context) (Decision, bool) {
	allowed := ctx.allowedReasons()
	if len(allowed) > 0 && !slices.Contains(allowed, ctx.Container.Reason) {
		klog.InfoS(
	...
	if len(forbidden) > 0 && slices.Contains(forbidden, ctx.Container.Reason) {
		klog.InfoS(
...
func repeatStatus(ctx *Context) Decision {
