...
func TestRunPodDeduplication(t *testing.T) {
	...
	ctrl := &Controller{
		components: componentsFor(&mockHandler{}),
	...
	assert.Equal(1, ctrl.components.Pod.Processor.(*mockHandler).podCount())
}
...
func TestMultipleWorkers(t *testing.T) {
	...
	assert.Equal(10, ctrl.components.Pod.Processor.(*mockHandler).podCount())
...
func TestBuildSeenSetSeedsNodeConditions(t *testing.T) {
	...
	expectedKey := incident.BuildKey("", "worker-1", "MemoryPressure", "")
	assert.NotContains(
		baseline,
		string(expectedKey),
