...
func (e *Engine) mergeNamespaceFanOut(
	...
	for _, scope := range scopes {
		...
		fanKey := groupKeyCodec.EncodeFanOut(
			entries[0].reason, entries[0].namespace,
		)
...
func (e *Engine) fanOutBuckets(ready []string) map[string][]string {
	...
	for _, gk := range ready {
		parsed, valid := groupKeyCodec.Parse(gk)
		if !valid || !parsed.OwnerForm || parsed.Namespace == "" ||
			parsed.Owner == "" {
			continue
		}
		...
		first := pg.entries[0]
		if first.namespace != parsed.Namespace || first.owner != parsed.Owner {
			continue
		}
		scope := parsed.Reason + "|" + parsed.Namespace
...
func (e *Engine) foldAnnouncedGroup(
	...
	parsed, _ := groupKeyCodec.Parse(gk)
	ns := parsed.Namespace
