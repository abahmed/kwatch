...
func BuildKey(namespace, owner, reason, container string) model.IncidentKey {
	return model.IncidentKey(
		namespace + ":" + owner + ":" + reason + ":" + container,
	)
}
...
type KeyParts struct {
	...
	IsMassFailure     bool // key is a mass-failure incident.
...
func ParseKey(key model.IncidentKey) KeyParts {
	...
	if strings.HasPrefix(s, groupKeyPrefix) {
		return KeyParts{
			IsGroup:  true,
			GroupKey: strings.TrimPrefix(s, groupKeyPrefix),
		}
	}
	if strings.HasPrefix(s, massFailureKeyPrefix) {
		return KeyParts{
			IsMassFailure:     true,
			MassDependencyKey: strings.TrimPrefix(s, massFailureKeyPrefix),
		}
	}
...
// globalSeparator delimits the cluster-scoped key form
// "<reason>|global|<scope>".
