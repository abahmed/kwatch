...
// observationReconciler remembers the reasons last reported for each watched
// object. Recovery is a domain concern: the incident engine owns identity,
// lifecycle, and the diff that closes reasons no longer present.
type observationReconciler struct {
	mu   sync.Mutex
	last map[model.ObjectRef]map[string]struct{}
}

func newObservationReconciler() *observationReconciler {
	return &observationReconciler{
		last: make(map[model.ObjectRef]map[string]struct{}),
	}
}

func (r *observationReconciler) diff(
	...
	for reason := range r.last[subject] {
		if _, present := current[reason]; !present {
...
func (r *observationReconciler) forget(subject model.ObjectRef) {
	r.mu.Lock()
	delete(r.last, subject)
	r.mu.Unlock()
}

// Reconcile processes all current observations for a subject and resolves
// reasons that disappeared since the previous pass. A no-finding pass
// resolves the subject as a whole, which also recovers incidents restored
// from persistence before this process observed them.
func (e *Engine) Reconcile(
	subject model.ObjectRef, observations []*model.Observation,
) {
	e.markProcessingStarted()
	gone := e.reconciler.diff(subject, observations)
	for _, obs := range observations {
		e.Process(obs)
	}
	...
	for _, reason := range gone {
		e.Resolve(subject, reason)
	}
}

// ReconcileGone removes the subject's observation history and closes the
// subject. Workload deletion also closes Pod symptoms keyed by that workload.
func (e *Engine) ReconcileGone(subject model.ObjectRef) {
	e.reconciler.forget(subject)
	e.Resolve(subject, "")
	if !workloadKinds[subject.Kind] {
		return
	}
	e.Resolve(model.ObjectRef{
		Kind: "pod", Namespace: subject.Namespace, Name: subject.Name,
	}, "")
}
...
// is what a caller in a monitor runtime would have to do, and it cannot: an
