...
// NOT hold e.mu; the source reference is taken under a short lock so this
// cannot race with source configuration.
func (e *Engine) precompute(ev event.Event) precomputed {
	e.mu.Lock()
	sources := e.attributionSources
	e.mu.Unlock()
	return precomputed{
		services: dependentServices(sources, ev.Namespace, ev.Labels),
	}
}
