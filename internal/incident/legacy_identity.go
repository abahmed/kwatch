...
func (e *Engine) migrateLegacyPodKey(
	key model.IncidentKey,
	inc *model.Incident,
) (model.IncidentKey, bool) {
	parts := ParseKey(key)
	if parts.IsGlobal || parts.IsGroup || parts.IsMassFailure ||
		inc == nil || inc.Resource != "pod" {
		return "", false
	}
	var match model.IncidentKey
	for candidate, pods := range e.baseline {
		candidateParts := ParseKey(model.IncidentKey(candidate))
		if candidateParts.Namespace != parts.Namespace ||
			candidateParts.Reason != parts.Reason {
			continue
		}
		if !strings.HasPrefix(candidateParts.Owner, "uid/") &&
			!strings.HasPrefix(candidateParts.Owner, "lineage/") {
			continue
		}
		if _, found := pods[inc.Name]; !found &&
			!containsIncidentResource(inc, pods) {
			continue
		}
		if match != "" {
			return "", false
...
func containsIncidentResource(
	inc *model.Incident,
	baselinePods map[string]int64,
) bool {
