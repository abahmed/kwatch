...
func TestPersistNilStateDoesNothing(t *testing.T) {
	cfg := &config.PvcMonitor{Enabled: true, Threshold: 80}
	incidentEngine := newTestIncidentEngine()
	m := newTestPvcMonitor(cfg, incidentEngine) // state == nil
...
func TestCloneSamplesDoesNotShareMap(t *testing.T) {
	original := map[string]model.PVCSample{
		"default/claim": {Namespace: "default", Name: "claim", Pct: 95},
	}
	clone := cloneSamples(original)
	clone["default/claim"] = model.PVCSample{Pct: 1}

	assert.Equal(t, 95.0, original["default/claim"].Pct)
}
