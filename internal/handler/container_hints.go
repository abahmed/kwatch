...
// spec, and the structured facts behind it. The hint is for people; the facts
// are what renderers use to build the OOM, probe and image sections — they
// used to regex the hint for the same information and broke whenever its
// wording changed.
func (h *handler) buildContainerHint(
	ctx *filter.Context,
) (string, model.Facts) {
	...
	if isOOM && h.oomTracker != nil {
		if s, f, repeating := h.repeatingOOMHint(ctx); repeating {
			return s, f
		}
	}
	...
	if spec != nil {
		if reason == constant.ReasonLivenessProbeFailed ||
		...
		} else if reason == constant.ReasonCrashLoopBackOff &&
			spec.LivenessProbe != nil {
	...
	hint, facts.PullSecretsSet = h.appendImagePullHint(hint, ctx, reason)
	return hint, facts
}
...
func (h *handler) repeatingOOMHint(
	ctx *filter.Context,
	...
	facts := model.Facts{
		OOMCount:     count,
		OOMWindowMin: h.config.OomMonitor.WindowMinutes,
		MemoryLeak:   true,
	}
	if timeline := h.oomTracker.History(key); timeline != "" {
		facts.OOMTimeline = "[" + timeline + "]"
		return fmt.Sprintf(
			"OOMKilled %d times in %dm — potential memory leak [%s]",
			count,
			h.config.OomMonitor.WindowMinutes,
			timeline,
		), facts, true
	}
	return fmt.Sprintf(
...
func (h *handler) oomHint(
	...
	if spec != nil && spec.Resources.Limits != nil {
		if mem := spec.Resources.Limits.Memory(); mem != nil && !mem.IsZero() {
			facts.MemoryLimit = mem.String()
			hint = fmt.Sprintf(
...
// The bool reports whether the pod declares imagePullSecrets, which the
// renderer shows as a fact.
func (h *handler) appendImagePullHint(
	hint string,
	ctx *filter.Context,
	reason string,
) (string, bool) {
	...
	if m := imagePullMsgHint(ctx.Container.Msg, hasSecrets); m != "" {
		return hint + "; " + m, hasSecrets
	}
	if !hasSecrets {
		// No specific pattern — check if registry likely needs auth.
		for _, c := range ctx.Pod.Spec.Containers {
			if c.Name == ctx.Container.Container.Name &&
				needsRegistryAuth(c.Image) {
				return hint + "; this image is from a registry that " +
					"typically requires authentication — add " +
					"imagePullSecrets to the pod spec", false
			}
		}
		return hint, false
	}
...
func buildProbeHint(reason string, spec *corev1.Container) string {
	...
	detail := reason
	if ep := probeEndpoint(reason, spec); ep != "" {
		detail = reason + " (" + ep + ")"
	}
	return fmt.Sprintf(
...
var imagePullPatterns = []imagePullPattern{
	...
	},
	{
		...
		hint: "Kubelet image pull QPS limit exceeded — consider increasing " +
			"registryPullQPS in kubelet config or reducing concurrent pod " +
			"starts",
	},
	{
		match: []string{
		...
		hint: "Registry authentication failed — check imagePullSecrets " +
			"validity",
	},
	{
		match: []string{
	...
	},
	{
		...
		hint: "Registry connection refused — check that the registry is " +
			"running and not blocked by a firewall",
	},
	{
		match: []string{"no route to host", "network is unreachable"},
		hint: "No network route to registry — check firewall rules and " +
			"network connectivity",
	},
	{
		match: []string{"no such host", "dial tcp"},
		hint: "Registry unreachable — check cluster network connectivity and " +
			"DNS",
	},
	{
