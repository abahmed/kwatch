...
func NewRetryConfigMapManager(
	client kubernetes.Interface,
	namespace string,
	configName string,
) *RetryConfigMapManager {
...
func (r *RetryConfigMapManager) UpdateWithRetry(
	...
	for i := 0; i < maxRetries; i++ {
		...
		if apierrors.IsNotFound(err) {
			...
			if _, cErr := r.client.CoreV1().ConfigMaps(
				r.namespace,
			).Create(ctx, cm, metav1.CreateOptions{}); cErr != nil {
