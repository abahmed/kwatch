...
// providerCatalogData is the guided installer's provider/field catalog, one
// record per line: provider|display|field|type|required|secret|validation|
// default|description, optionally followed by group|condition.
//
// It lives in its own file because it is data, not logic: keeping it beside
// the parser pushed that file past twice the size limit and buried ninety
// lines of code in seven hundred lines of payload. cmd/providercatalog
// renders it to deploy/provider-catalog.tsv, and make verify-catalogs
// byte-compares the result, so any edit here must be regenerated there.
