# Third-party notices

Kwatch includes code from the third-party packages below. Kwatch does not
relicense these packages; each package keeps the license shown in the table.
The links point to the license or notice in the exact source revision used by
the current `go.mod`/`go.sum` dependency graph.

This inventory was generated from the packages reachable by `go-licenses` and
should be regenerated when dependencies change. Package-level entries are
listed where a dependency contains a separately licensed fork or subpackage.

## Apache-2.0

- [github.com/go-logr/logr](https://github.com/go-logr/logr/blob/v1.4.4/LICENSE)
- [github.com/go-openapi/jsonpointer](https://github.com/go-openapi/jsonpointer/blob/v1.0.0/LICENSE)
- [github.com/go-openapi/jsonreference](https://github.com/go-openapi/jsonreference/blob/v1.0.1/LICENSE)
- [github.com/go-openapi/swag](https://github.com/go-openapi/swag/blob/v0.29.1/LICENSE) and its subpackages
- [github.com/google/gnostic-models](https://github.com/google/gnostic-models/blob/v0.7.1/LICENSE)
- [github.com/modern-go/concurrent](https://github.com/modern-go/concurrent/blob/bacd9c7ef1dd/LICENSE)
- [github.com/modern-go/reflect2](https://github.com/modern-go/reflect2/blob/35a7c28c31ee/LICENSE)
- [go.yaml.in/yaml/v2](https://github.com/yaml/go-yaml/blob/v2.4.4/LICENSE)
- [k8s.io/api](https://github.com/kubernetes/api/blob/v0.37.0/LICENSE)
- [k8s.io/apimachinery](https://github.com/kubernetes/apimachinery/blob/v0.37.0/LICENSE)
- [k8s.io/client-go](https://github.com/kubernetes/client-go/blob/v0.37.0/LICENSE)
- [k8s.io/klog/v2](https://github.com/kubernetes/klog/blob/v2.140.0/LICENSE)
- [k8s.io/kube-openapi](https://github.com/kubernetes/kube-openapi/blob/be32def86098/LICENSE)
- [k8s.io/utils](https://github.com/kubernetes/utils/blob/cf1189d6abe3/LICENSE)
- [sigs.k8s.io/json](https://github.com/kubernetes-sigs/json/blob/2d320260d730/LICENSE)
- [sigs.k8s.io/randfill](https://github.com/kubernetes-sigs/randfill/blob/v1.0.0/LICENSE)
- [sigs.k8s.io/structured-merge-diff/v6](https://github.com/kubernetes-sigs/structured-merge-diff/blob/v6.4.2/LICENSE)
- [sigs.k8s.io/yaml](https://github.com/kubernetes-sigs/yaml/blob/v1.6.0/LICENSE)

## BSD-3-Clause

- [github.com/ProtonMail/go-crypto](https://github.com/ProtonMail/go-crypto/blob/7d5c6f04bbb8/LICENSE)
- [github.com/bwmarrin/discordgo](https://github.com/bwmarrin/discordgo/blob/v0.29.0/LICENSE)
- [github.com/cloudflare/circl](https://github.com/cloudflare/circl/blob/v1.6.3/LICENSE)
- [github.com/google/go-github/v55](https://github.com/google/go-github/blob/v55.0.0/LICENSE)
- [github.com/google/go-querystring](https://github.com/google/go-querystring/blob/v1.2.0/LICENSE)
- [github.com/google/uuid](https://github.com/google/uuid/blob/v1.6.0/LICENSE)
- [github.com/munnerz/goautoneg](https://github.com/munnerz/goautoneg/blob/a7dc8b61c822/LICENSE)
- [github.com/pmezard/go-difflib](https://github.com/pmezard/go-difflib/blob/5d4384ee4fb2/LICENSE)
- [github.com/spf13/pflag](https://github.com/spf13/pflag/blob/v1.0.10/LICENSE)
- [golang.org/x/crypto](https://cs.opensource.google/go/x/crypto/+/v0.55.0:LICENSE)
- [golang.org/x/exp](https://cs.opensource.google/go/x/exp/+/e88cd736:LICENSE)
- [golang.org/x/net](https://cs.opensource.google/go/x/net/+/v0.58.0:LICENSE)
- [golang.org/x/oauth2](https://cs.opensource.google/go/x/oauth2/+/v0.36.0:LICENSE)
- [golang.org/x/sys](https://cs.opensource.google/go/x/sys/+/v0.47.0:LICENSE)
- [golang.org/x/term](https://cs.opensource.google/go/x/term/+/v0.45.0:LICENSE)
- [golang.org/x/text](https://cs.opensource.google/go/x/text/+/v0.41.0:LICENSE)
- [golang.org/x/time](https://cs.opensource.google/go/x/time/+/v0.15.0:LICENSE)
- [google.golang.org/protobuf](https://github.com/protocolbuffers/protobuf-go/blob/v1.36.12/LICENSE)
- [gopkg.in/evanphx/json-patch.v4](https://github.com/evanphx/json-patch/blob/v4.13.0/LICENSE)
- [gopkg.in/inf.v0](https://github.com/go-inf/inf/blob/v0.9.1/LICENSE)
- Kubernetes forked Go packages under `third_party/forked` retain their upstream BSD-3-Clause notices.

## BSD-2-Clause

- [github.com/gorilla/websocket](https://github.com/gorilla/websocket/blob/e064f32e3674/LICENSE)
- [github.com/slack-go/slack](https://github.com/slack-go/slack/blob/v0.29.0/LICENSE)

## MIT

- [github.com/emicklei/go-restful/v3](https://github.com/emicklei/go-restful/blob/v3.13.0/LICENSE)
- [github.com/fxamacker/cbor/v2](https://github.com/fxamacker/cbor/blob/v2.9.3/LICENSE)
- [github.com/json-iterator/go](https://github.com/json-iterator/go/blob/v1.1.12/LICENSE)
- [github.com/robfig/cron/v3](https://github.com/robfig/cron/blob/v3.0.1/LICENSE)
- [github.com/x448/float16](https://github.com/x448/float16/blob/v0.8.4/LICENSE)
- [go.yaml.in/yaml/v3](https://github.com/yaml/go-yaml/blob/v3.0.5/LICENSE)
- [gopkg.in/mail.v2](https://github.com/go-mail/mail/blob/v2.3.1/LICENSE)
- [gopkg.in/yaml.v3](https://github.com/go-yaml/yaml/blob/v3.0.1/LICENSE)

## ISC

- [github.com/davecgh/go-spew](https://github.com/davecgh/go-spew/blob/d8f796af33cc/LICENSE)

## Notice handling

The full license text for each dependency is available at its linked source
location. When a dependency is upgraded, the release owner must refresh this
file and verify any newly introduced license before publishing the release.
